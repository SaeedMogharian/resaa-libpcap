...
void packet_handler(u_char *user, const struct pcap_pkthdr *packethdr, const u_char *packetptr) {
    ...
    // Inject packet into NIC B
    if (pcap_inject(handle_inject, packetptr - linkhdrlen, packet_size) == PCAP_ERROR) {
        fprintf(stderr, "pcap_inject failed: %s\n", pcap_geterr(handle_inject));
    }
}
...
void stop_capture(int signo) {
    ...
    if (pcap_stats(handle_sniff, &stats) >= 0) {
        printf("\n%d packets captured\n", packets);
...
{
    char device_sniff[256];
    char device_inject[256];
    char filter[256]; 
    int count = 0;
    int opt;
    ...
    *filter = 0;
    ...
    while ((opt = getopt(argc, argv, "hi:j:n:")) != -1)
    {
        ...
        {
        case 'h':
            printf("usage: %s [-h] [-i interface_sniff] [-j interface_inject] [-n count] [BPF expression]\n", argv[0]);
            exit(0);
            break;
        case 'i':
            strcpy(device_sniff, optarg);
            break;
        case 'j':
            strcpy(device_inject, optarg);
            break;
        case 'n':
            count = atoi(optarg);
            break;
    ...
    handle_sniff = create_pcap_handle(device_sniff, filter);
    if (handle_sniff == NULL) {
        return -1;
    }

    // Create packet injection handle for injecting.
    handle_inject = create_pcap_handle(device_inject, "");
    ...
    get_link_header_len(handle_sniff);
    if (linkhdrlen == 0) {
        pcap_close(handle_sniff);
        pcap_close(handle_inject);
        return -1;
    ...
    if (pcap_loop(handle_sniff, count, packet_handler, (u_char*)NULL) == PCAP_ERROR) {
        fprintf(stderr, "pcap_loop failed: %s\n", pcap_geterr(handle_sniff));
        pcap_close(handle_sniff);
        pcap_close(handle_inject);
        return -1;
    }
